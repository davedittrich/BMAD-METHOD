...
3. **Context Persistence**
   - Store active project context in `.bmad/config/active-project.json` for session persistence
...
4. **Integration with Enhanced Tasks**
   - Active project context automatically applies to project-aware tasks.
   - Tasks that handle project-specific data (e.g., `save-transcript`) will use the active context when no explicit project parameter is provided.
