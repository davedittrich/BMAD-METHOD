...
1. Reading `.bmad/config/active-project.json` if it exists
2. Validating the active project still exists in `.bmad/projects/{active-project}/`
...
_Use project-status task for complete project inventory_
```
...
**Output Location:** `.bmad/projects/[project-name]/[analyses|references|drafts]/[filename]`  
...
_To change project context, use `project-switch` task_
```
...
1. Check for `.bmad/config/active-project.json`
